	...
	try {
		...
		for (const card of cards) {
			...
			for (const line of lines) {
				...
				if (/^q\s*:/i.test(normalizedLine)) {
					question = normalizedLine.replace(/^q\s*:/i, "").trim();
				} else if (/^a\s*:/i.test(normalizedLine)) {
					answer = normalizedLine.replace(/^a\s*:/i, "").trim();
				} else if (/^d\s*:/i.test(normalizedLine)) {
					const diff = normalizedLine
						.replace(/^d\s*:/i, "")
						.trim()
						.toLowerCase();
					if (["easy", "medium", "meduim", "hard"].includes(diff)) {
						difficulty = diff === "meduim" ? "medium" : diff;
					}
				}
			}
			if (question && answer) {
		...
		if (flashcards.length === 0) {
			throw new Error("No flashcards could be parsed from Gemini output");
		}
